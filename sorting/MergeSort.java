...
public class MergeSort {
    ...
    private static void merge (Song [] a, Song[] aux, int lo , int mid, int hi){
      // k is our current entry on the sorted array  
        for (int k=lo; k<=hi; k++)
            aux [k] = a [k] ; //copy everything from the original array into an auxillary array

        int i =lo , j= mid + 1; //i is the first part of our array and j is the second part of our array
        for (int k =lo ; k<=hi; k++){
            if (i>mid)                  aux[k] = a[j++]; //if i is exhausted, move over to the j element
            else if (j>hi)              aux[k] = a[i++]; // if j is exhausted, move over to the i element
            else if(aux[j].getViews() < aux[i].getViews())    a[k] = aux[j++];     // compare the minimum in each sub array and if an element from j is less than an element from i the element is taken back to its original array and sub array j with original array k are both incremented.
            else                        a[k] = a[i++];// else an element from i will be less than an element from j hence element i taken back to its original array and sub array i with original array k are incremented.
        }
    ...
    private static void sort (Song[]a, Song[]aux, int lo, int hi){
     ...
     public static void sort (Song[]a){ //Actual array creates an auxillary array that it uses
         aux = new Song [a.length];
